...
Solid provides a way to load data from your data sources using the [`createResource` primitive](/solid-js/reference/basic-reactivity/create-resource).
It takes an async function and returns a [signal](/solid-js/reference/basic-reactivity/create-signal) from it.
`createResource` integrates with [`Suspense`](/solid-js/reference/components/suspense) and [`ErrorBoundary`](/solid-js/reference/components/error-boundary) to help manage lifecycle and error states.
