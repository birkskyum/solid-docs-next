...
When you have a large [component tree](/solid-js/concepts/components/basics#component-trees) that requires state to be shared, context can be used.
Context can be employed to avoid [prop drilling](/solid-js/concepts/components/props#prop-drilling), which is the practice of passing props through intermediate elements without using them directly.
...
[Signals](/solid-js/concepts/signals) are often a simplest solution since they can be imported directly into the components that need them.
...
Context is created using the [`createContext`](/solid-js/reference/component-apis/create-context) function.
...
Once the values are available to all the components in the context's component tree, they can be accessed using the [`useContext`](/solid-js/reference/component-apis/use-context) utility.
...
[Signals](/solid-js/concepts/signals) offer a way to synchronize and manage data shared across your components using context.
