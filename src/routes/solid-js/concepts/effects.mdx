...
This is useful for setting up the effect and initializing variables or subscribing to [signals](/solid-js/concepts/signals).
...
In situations where you just want to run a side effect **once**, you can use the [`onMount`](/solid-js/reference/lifecycle/on-mount) function.
...
While `onMount` is useful for running a side effect once, [`onCleanup`](/solid-js/reference/lifecycle/on-cleanup) is helpful for cleaning up a task when it is no longer needed.
