...
Similar to [signals](/solid-js/concepts/signals), stores are state management primitive.
...
While signals are created using the [`createSignal`](/solid-js/reference/basic-reactivity/create-signal) function and require calling the signal function to access their values, store values can be directly accessed without a function call.
...
Rather, this would need to be in a tracking scope, like inside a [`createEffect`](/solid-js/reference/basic-reactivity/create-effect), so that a dependency is established.
...
To learn more about how to use Stores in practice, visit the [guide on complex state management](/solid-js/guides/complex-state-management).
