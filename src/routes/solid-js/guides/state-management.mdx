...
State is represented by a [signal](/solid-js/concepts/signals), which is a reactive primitive that manages state and notifies the UI of any changes.

To create a piece of state, you use the [`createSignal`](/solid-js/reference/basic-reactivity/create-signal) function and pass in the initial value of the state:
...
Additionally, signals are reactive, which means as long as it is accessed within a [tracking scope](/solid-js/concepts/intro-to-reactivity#tracking-changes), it will always be up to date.
...
This is known as [fine-grained reactivity](/solid-js/advanced-concepts/fine-grained-reactivity).
...
This can be achieved through the use of [effects](/solid-js/concepts/effects), which are reactive primitives that perform side effects when the state changes:
...
The [`createEffect`](/solid-js/reference/basic-reactivity/create-effect) function sets up a function to perform side effects whenever the state is modified.
...
Derived values can be created through using a signal within a function, which can be referred to as a [derived signal](/solid-js/concepts/derived-values/derived-signals):
...
To avoid this, Solid introduced [Memos](/solid-js/concepts/derived-values/memos):
...
Using the [`createMemo`](/solid-js/reference/basic-reactivity/create-memo) function, you can create a memoized value that is only re-evaluated when its dependencies change.
...
This can keep things synchronized across the [component tree](/solid-js/concepts/components/basics#component-trees) and allow for more predictable state management.
...
When sharing state between components, you can access the state through [`props`](/solid-js/concepts/components/props).
...
There are [specific utility functions for props](/solid-js/concepts/components/props), however, that offer methods to modify props values.
...
To avoid the concept of prop drilling, which is the process of passing props through multiple components, Solid offers [stores](/solid-js/concepts/stores) to manage state in a more scalable and maintainable manner.

To learn more about managing complex state, navigate to the [complex state management page](/solid-js/guides/complex-state-management).
