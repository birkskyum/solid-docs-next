...
As applications grow and start to involve many components, more intricate user interactions, and possibly communication with backend services, you may find that staying organized with more [basic state management methods](/solid-js/guides/state-management) can become difficult to maintain.
...
If you're new to the concept stores, there see the [stores section](/solid-js/concepts/stores).
...
To create the signal so it reactively updates, you have to access the property within a tracking scope, such as using a [`createEffect`](/solid-js/reference/basic-reactivity/create-effect):
...
The setter in stores follow [path syntax](/solid-js/concepts/stores#path-syntax-flexibility): `setStore("key", value)`.
...
In situations where you need to make multiple `setState` calls and target multiple properties, you can simplify your code and improve readability by using Solid's [`produce`](/solid-js/concepts/stores#store-updates-with-produce) utility function.
...
Read about some of the other [advantages to using `produce`](/solid-js/concepts/stores#store-updates-with-produce).
...
To solve this problem and allow for a more scalable and maintainable codebase, Solid provides [context](/solid-js/concepts/context).
...
For a deeper dive, please refer to our dedicated [page on context](/solid-js/concepts/context).
