...
This primtive can be especially useful with other Solid primitives, such as [`createResource`](/solid-js/reference/basic-reactivity/create-resource) and [`createSignal`](/solid-js/reference/basic-reactivity/create-signal), which can create dynamic behaviours based on the route parameters.
...
This adjustment tells your application to use the URL hash to manage routes, which can be useful for ensuring compatibility with browsers that do not support the [History API](https://developer.mozilla.org/en-US/docs/Web/API/History_API) or for serving static files.
...
The `useParams` primitive retrieves a reactive object containing the current route path parameters as defined in the Route.
...
`useMatch` creates a [Memo](/solid-js/concepts/derived-values/memos) that returns match information if the current path matches the provided path, which is useful for determining if a given path matches the current route.
