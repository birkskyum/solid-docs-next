...
**Note:** If you're new to the concept of reactivity and want to learn the basics, consider starting with our [intro to reactivity guide](/solid-js/concepts/intro-to-reactivity).
...
- [Stores](/solid-js/concepts/stores) which are [proxies](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy) that create, read, and write signals under the hood.
- [Memos](/solid-js/concepts/derived-values/memos) resemble [effects](/solid-js/concepts/effects) but are distinct in that they _return_ a signal and optimize computations through caching.
  They update based on the behavior of effects, but are more ideal for computational optimization.
- [Resources](/solid-js/guides/fetching-data), building on the concept of memos, convert the asynchronicity of network requests into synchronicity, where the results are embedded within a signal.
...
[Signals](/solid-js/concepts/signals) are like mutable variables that can point to a value now and another in the future.
...
In Solid, the [`createSignal`](/solid-js/reference/basic-reactivity/create-signal) function can be used to create a signal.
...
[Effects](/solid-js/concepts/effects) are functions that are triggered when the signals they depend on point to a different value.
